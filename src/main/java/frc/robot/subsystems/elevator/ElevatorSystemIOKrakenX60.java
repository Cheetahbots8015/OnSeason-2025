...
public class ElevatorSystemIOKrakenX60 implements ElevatorSystemIO {
	private final TalonFX leader = new TalonFX(ElevatorConstants.ELEVATOR_LEADER_ID, ElevatorConstants.ELEVATOR_CANNAME);
	...
	public ElevatorSystemIOKrakenX60() {
		...
		// config motionmagic
		leaderConfigs.MotionMagic.MotionMagicCruiseVelocity = ElevatorConstants.ELEVATOR_LEADER_CRUISE_VELOCITY;
		leaderConfigs.MotionMagic.MotionMagicAcceleration = ElevatorConstants.ELEVATOR_LEADER_CRUISE_ACCELERATION;
		...
		leaderConfigs.MotorOutput.withPeakForwardDutyCycle(ElevatorConstants.ELEVATOR_FORWARD_DUTY_CYCLE_LIMIT);
		leaderConfigs.MotorOutput.withPeakReverseDutyCycle(ElevatorConstants.ELEVATOR_REVERSE_DUTY_CYCLE_LIMIT);
		followerConfigs.MotorOutput.withPeakForwardDutyCycle(ElevatorConstants.ELEVATOR_FORWARD_DUTY_CYCLE_LIMIT);
		followerConfigs.MotorOutput.withPeakReverseDutyCycle(ElevatorConstants.ELEVATOR_REVERSE_DUTY_CYCLE_LIMIT);
		...
		followerConfigs.DifferentialSensors.DifferentialTalonFXSensorID = ElevatorConstants.ELEVATOR_LEADER_ID;
	...
	public void setPosition_MotionMagician(double position) {
		if (Math.abs(this.getLeaderPositionWithoutOffset() - position)
			...
			setVoltage((filter.calculate(ElevatorConstants.ELEVATOR_MOTION_MAGICIAN_HIGH_DOWN_VOLTAGE_L3L4)));
		} else if (this.getLeaderPositionWithoutOffset()
				< position - ElevatorConstants.ELEVATOR_MOTION_MAGICIAN_CLOSE_POSITION_DEADBAND_L3L4) {
			...
			setVoltage(filter.calculate(ElevatorConstants.ELEVATOR_MOTION_MAGICIAN_LOW_DOWN_VOLTAGE_L3L4));
		} else {
	...
	public void defaultFall(){
		if (this.getLeaderPositionWithoutOffset() > ElevatorConstants.ELEVATOR_DEFAULT_SHUT_DOWN_POSITION) {
