...
public class ElevatorSystemIOKrakenX60 implements ElevatorSystemIO {
  ...
  public ElevatorSystemIOKrakenX60() {
    ...
    leaderConfigs.OpenLoopRamps.VoltageOpenLoopRampPeriod =
        ElevatorConstants.ELEVATOR_OPEN_LOOP_RAMP_PERIOD;
  ...
  public void setPosition(double position) {
    ...
    if (lastPosition < (position + encoderOffset)) {
      if ((leader.getPosition().getValueAsDouble() - lastPosition)
          < (position + encoderOffset - lastPosition) / 2) {
      ...
      if ((lastPosition - leader.getPosition().getValueAsDouble())
          < (lastPosition - position - encoderOffset) / 2) {
