...
public class RobotContainer {
    ...
    private final CommandXboxController operatorController = new CommandXboxController(
            JoystickConstants.operatorControllerPort);

    // driver triggers
    private final Trigger DriverL1Trigger = driverController.a().and(driverController.rightBumper().negate());
    private final Trigger DriverL2Trigger = driverController.b().and(driverController.rightBumper().negate());
    private final Trigger DriverL3Trigger = driverController.x().and(driverController.rightBumper().negate());
    private final Trigger DriverL4Trigger = driverController.y().and(driverController.rightBumper().negate());
    private final Trigger DriverStationTrigger = driverController.leftTrigger();
    private final Trigger DriverLowAlgaeTrigger = driverController.a().and(driverController.rightBumper());
    private final Trigger DriverHighAlgaeTrigger = driverController.y().and(driverController.rightBumper());
    private final Trigger DriverProcessorTrigger = driverController.b().and(driverController.rightBumper());
    ...
    // driver commands
    private final Command DriverL1Command = new L1Command(m_elevatorSubsystem, m_pivotSubsystem, m_rollerSubsystem);
    private final Command DriverL2Command = new L2Command(m_elevatorSubsystem, m_pivotSubsystem, m_rollerSubsystem);
    private final Command DriverL3Command = new L3Command(m_elevatorSubsystem, m_pivotSubsystem, m_rollerSubsystem);
    private final Command DriverL4Command = new L4Command(m_elevatorSubsystem, m_pivotSubsystem, m_rollerSubsystem);
    private final Command DriverLowAlgaeCommand = new LowAlgaeCommand(m_rollerSubsystem, m_pivotSubsystem,
            m_elevatorSubsystem);
    private final Command DriverHighAlgaeCommand = new HighAlgaeCommand(m_rollerSubsystem, m_pivotSubsystem,
            m_elevatorSubsystem);
    private final Command DriverProcessorCommand = new ProcessorCommand(m_rollerSubsystem, m_pivotSubsystem,
            m_elevatorSubsystem);
    ...
    private final Command OperatorElevatorUpCommand = new ElevatorManualUpCommand(m_elevatorSubsystem, m_pivotSubsystem,
            m_rollerSubsystem);
    private final Command OperatorElevatorDownCommand = new ElevatorManualDownCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorPivotForwardCommand = new PivotManualForwardCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorPivotReverseCommand = new PivotManualReverseCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorManualRollerForwardCommand = new RollerManualForwardCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorManualRollerReverseCommand = new RollerManualReverseCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorNormalRollerForwardCommand = new RollerNormalForwardCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);
    private final Command OperatorNormalRollerReverseCommand = new RollerNormalReverseCommand(m_elevatorSubsystem,
            m_pivotSubsystem, m_rollerSubsystem);

    // default commandfs
    private final Command ElevatorDefaultIdleCommand = new ElevatorDefaultIdleCommand(m_elevatorSubsystem);
    ...
    public RobotContainer() {
        ...
        CameraServer.startAutomaticCapture("Reef", "http://limelight-reef.local:5800/");
        CameraServer.startAutomaticCapture("Station", "http://limelight-station.local:5800/");
    ...
    private void configureBindings() {
        ...
        drive.setDefaultCommand(
                DriveCommands.joystickDrive(
                        drive,
                        driverController,
                        driverController.rightTrigger().getAsBoolean() ? () -> Math.pow(-driverController.getLeftY(), 3)
                                : () -> -driverController.getLeftY(),
                        driverController.rightTrigger().getAsBoolean() ? () -> Math.pow(-driverController.getLeftX(), 3)
