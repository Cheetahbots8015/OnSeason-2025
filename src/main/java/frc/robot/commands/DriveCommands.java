...
public class DriveCommands {
  ...
  public static Command Rotate2AprilTag(Drive drive) {
    return Commands.run(
        () -> {
          LimelightHelpers.setCameraPose_RobotSpace(null, 0.19011, 0.24657, 0.82655, 0, -20, 0);
          boolean hasTarget = LimelightHelpers.getTV("");
          PIDController pidx = new PIDController(2, 0, 0);
          ...
          if (hasTarget) {
            ChassisSpeeds speeds =
                new ChassisSpeeds(
                    pidx.calculate(pose.getTranslation().getY()),
                    pidy.calculate(pose.getTranslation().getX()),
                    pidyaw.calculate(pose.getRotation().getX()));
            boolean isFlipped =
                DriverStation.getAlliance().isPresent()
                    && DriverStation.getAlliance().get() == Alliance.Red;
            drive.runVelocity(
